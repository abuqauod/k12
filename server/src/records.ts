...
/**
 * Plumbing shared by the record-keeping modules (finance Phase 3, HR,
 * operations): a guarded route, a transaction whose typed aborts come out
 * as failures, one error-to-status map, and the caller's branch filter.
 */
...
/** Runs `fn` in one transaction; an `Abort`, or a unique-index clash
 * (reported as `duplicate`, by default a second pending approval for the
 * same thing), rolls it back and comes out as a failure. */
export async function transact<T>(
  ...
  try {
    ...
    if (error instanceof Abort) return { ok: false, error: error.code, extra: error.extra }
    if (isDuplicateKey(error)) return { ok: false, error: duplicate }
    throw error
  }
}
...
/** Error codes with a status other than 409 (the default for a failed
 * business rule). */
...
/** Minor units as the UI shows them ("850.00"). */
